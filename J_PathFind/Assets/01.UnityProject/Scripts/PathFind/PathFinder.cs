...
public class PathFinder : GSingleton<PathFinder>
{
    ...
    private IEnumerator DelayFindPath_Astar(float delay_)
    {
        ...
        while (isFoundDestination == false && isNowayToGo == false)
        {
            ...
            foreach (var terrainNode in aStarOpenPath)
            {
                ...
                }   //else: 가장 작은 코스트이 노드가 캐싱되어 잇는 경우
            }   //loop: 가장 코스트가 낮은 노드를 낮는 루프
            //}Open list를 순회해서 가장 코스트가 낮은 노드를 선택한다.
            ...
            if (isArriveDest)
            {
                //{목적지에 도착했다면 aStarResultPaht 리스트를 설정한다.
                ...
                }       //loop: 이전 노드가 찾지 못할 때까지 순회하는 루프
                //}목적지에 도착했다면 aStarResultPaht 리스트를 설정한다.

                //Open list와 Close list 를 정리한다.
                aStarOpenPath.Clear();
    ...
    private void Add_AstarOpenList(AstarNode targetTerrain_, AstarNode prevNode = default)
    {
        ...
        else
        {
            ...
            {
                //타켓 노드의 코스트가 더 작은 경우에는 Opne list 에서 노드를 교체한다.
                //타켓 노드의 코스트가 더 큰 경우에는 Open list에 추가하지 않는다.
            ...
            {
                aStarOpenPath.Add(targetTerrain_);
            }   //else: open list에 현재 추가할 노드와 같은 좌표의 노드가 없는겨우
        ...
        AstarNode targetNode, AstarNode prevNode)
    {
        ...
        Vector2Int distance2D = mapBoard.
        GetDistance2D(targetNode.Terrain.gameObject, destinationObj);
